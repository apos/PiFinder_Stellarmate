...
4. `launch` sets `PIFINDER_WEB_PORT` (default 8081, see `--port`) and the API
   binds directly to that port - deliberately not 8080, since that's where a
   real, already-running `pifinder.service` normally ends up via its own
   port-80-busy fallback. Pass `--base-url http://host:port` to override.
