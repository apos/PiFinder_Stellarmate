...
DEFAULT_PORT = 8081  # deliberately not 8080: that's where a real, already-
# running pifinder.service normally ends up (its own port-80-busy fallback)
CEDAR_PORT = 50551
...
def cmd_launch(args):
    ...
    # server.py's Server.run() honors PIFINDER_WEB_PORT (set below) and binds
    # directly to it, skipping its own 80-then-8080 fallback entirely - so
    # there's exactly one candidate to probe, deterministically, instead of
    # guessing across 80/8080 (which also risks a false-positive match
    # against an unrelated service already answering on one of those ports,
    # e.g. a reverse proxy/dashboard that returns 200 for any path).
    if args.base_url:
        candidates = [args.base_url.rstrip("/")]
    else:
        candidates = [f"http://127.0.0.1:{args.port}"]
    ...
    pf_env["PIFINDER_WEB_PORT"] = str(args.port)
...
def build_parser():
    ...
    p.add_argument("--port", type=int, default=DEFAULT_PORT, help=f"API port if no base URL (default {DEFAULT_PORT}); for `launch`, also passed to PiFinder as PIFINDER_WEB_PORT")
