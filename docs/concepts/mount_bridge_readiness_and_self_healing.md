...
| 2.5 | (2) is healthy | The Web Manager's own persisted profile record still lists "PiFinder Mount Bridge" as a driver (`GET /api/profiles/{p}/labels`) | Re-add it there via the plain-replace endpoint (`set_pifinder_bridge(profile, True)`) - bookkeeping only, never touches the already-running driver. Found live 2026-09-12: the driver can be fully running/connected/linked while this persisted record has fallen out of sync (root cause not fully pinned down - the removal path is a delete-and-recreate of the whole profile, and nothing on that path logs who/what triggered it) - `deriveProfileRole()` then misreads the profile as plain "PiFinder host" and the whole coupling UI disappears despite the driver working fine. Debounced the same way as (1) - must not fight a genuine, in-progress removal (switching roles on purpose) while the old process is still shutting down. |
