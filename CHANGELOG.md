...
- **Mount Bridge: "Goto Held Target" manual button**: the existing "Sync Now"/"Goto Now" manual
  ...
  is rigidly mounted to the OTA and its live solve reflects the disturbance too. The new third
  button in the same `MANUAL_TRIGGER` switch group instead re-precesses and re-sends the held
