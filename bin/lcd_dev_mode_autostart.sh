...
# (camera/keyboard/display all stubbed - no GPIO clash) plus the screen
# mirror, so the LCD actually shows something without a manual step after
# every reboot. The numpad-to-keypad bridge (fb_keyboard_bridge.py) is
# NOT started here - it needs no GPIO/overlay at all (just a USB/2.4GHz
# dongle numpad via evdev) and works the same in Real Mode, so it's its
# own independent Control Center toggle instead (see basic-memory/
# pifinder-stellarmate/00031). See also 00030 for this script's own history.
set -u
...
echo "Waiting for Fake Mode's API to answer before starting the screen mirror..."
up=0
...
if [ "$up" -ne 1 ]; then
    echo "ERROR: Fake Mode API never answered - not starting the screen mirror." >&2
    exit 1
fi
