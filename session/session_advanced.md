...
-   **KStars Log Management Details**: The script clears `~/.local/share/kstars/logs/*` before the 30-second wait. This ensures that only logs relevant to the current test session are captured. The log file is then copied to a timestamped file in `.gemini/tmp/` for easy access and review.
