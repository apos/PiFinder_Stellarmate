...
-   **KStars Log Management Details**: The script now includes an interactive `read -p` prompt and a `sleep 30` command, allowing the user to initiate KStars and the INDI server before log capture. After the wait, it searches for logs within the `/home/stellarmate/.indi/logs/` directory, specifically looking for files in the `indi_pifinder_lx200` subdirectory. It dynamically finds the *newest log file* and appends it to the build log for easy access and review, without deleting any existing logs.
