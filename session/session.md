...
- **`bin/build_indi_driver.sh`**: Heavily refactored for speed, safety, and debuggability.
    ...
    -   **Why (KStars Log Management):** This functionality was restored to include an interactive prompt and a 30-second wait, allowing for manual driver testing. After the wait, the script now robustly identifies the *latest* log file by searching in both the `/home/stellarmate/.indi/logs/` and `/home/stellarmate/.local/share/kstars/logs/` locations, and appends its content to the build log, without deleting any existing logs. This is vital for debugging driver behavior.
