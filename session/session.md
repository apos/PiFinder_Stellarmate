...
- **`bin/build_indi_driver.sh`**: Heavily refactored for speed, safety, and debuggability.
    ...
    -   **Why (KStars Log Management):** This functionality was restored and improved to clear old KStars log *directories*, wait for a testing period, and then copy the *latest* log file (identified dynamically from the new `/home/stellarmate/.indi/logs/` location) for easy inspection, which is vital for debugging driver behavior.
